...
// Define schema here (can also be imported from @/lib/validations/werkgever.schema)
const WerkgeverLeadSchema = z.object({
...
type FormData = z.infer<typeof WerkgeverLeadSchema>;
...
const WerkgeverLeadForm: React.FC<WerkgeverLeadFormProps> = ({
  answers,
  onSubmit,
}) => {
  const methods = useForm<FormData>({
    resolver: zodResolver(WerkgeverLeadSchema),
    defaultValues: {
      name: '',
      company: '',
      email: '',
    ...
    <FormProvider {...methods}>
      <form onSubmit={methods.handleSubmit(onSubmitForm)} className="space-y-6">
        ...
        <div className="space-y-2">
          ...
          {methods.formState.errors.company && (
            <p className="text-red-500 text-sm mt-1">
              {methods.formState.errors.company.message}
            </p>
          )}
        </div>
